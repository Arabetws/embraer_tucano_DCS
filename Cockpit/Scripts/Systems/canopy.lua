...
function update()
	...
	elseif (CANOPY_COMMAND == 1 and current_canopy_position <= 0.89) then
		...
		current_canopy_position = current_canopy_position + 0.01
    elseif current_canopy_position < 0 then 
        current_canopy_position = 0
        canopy_snd:play_once()
        canopy_move_snd:stop()
    elseif current_canopy_position >= 0.89 and current_canopy_position < 0.95 then 
    ...
    if prev_canopy_val ~= cockpit_lever then
        local canopy_lever_clickable_ref = get_clickable_element_reference("PNT_26")
    ...
    if current_canopy_position > 0 and canopy_warning == 0 then 
        set_warning(WARNING_ID.CANOPY,2) 
        canopy_warning = 1
