...
function update()
    ...
    if is_engine_on_fire() and fire_warning == 0 then
        set_warning(WARNING_ID.FIRE, 2)
        fire_warning = 1
    elseif not is_engine_on_fire() and fire_warning == 1 then
...
function post_initialize()
    ...
    throttle_clickable_ref = get_clickable_element_reference("PNT_80")
...
function engine_try_start()
    local bat_sw=get_cockpit_draw_argument_value(1001) == 1
    local ign_sw=get_cockpit_draw_argument_value(1030) >= 0
    local fuelbld_sw=get_cockpit_draw_argument_value(1920) <= 0
    local fuelmainpump_sw = (get_cockpit_draw_argument_value(1204) == 1 or get_cockpit_draw_argument_value(12021) == 1)
    
    if bat_sw and ign_sw and fuelbld_sw then
